...
!     This is an internal dictionary message that was
!     generated by the BUFRLIB archive library software.

      IF(ISC3(LUN).NE.0) RETURN

!     Section 3 decoding isn't being used, so backspace the
!     file pointer and then use subroutine RDBFDX to read in
!     all such dictionary messages (they should be stored
!     consecutively!) and reset the internal tables.
