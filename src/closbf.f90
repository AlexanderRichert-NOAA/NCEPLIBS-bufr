!> @file
!> @brief Close a previously opened system file and disconnect it from
!> the BUFRLIB software.

!> This subroutine closes the connection between logical unit
!> LUNIT and the BUFRLIB software.
!>
!> @authors J. Woollen
!> @authors J. Ator
!> @date 1994-01-06
...
!>
!> @remarks
!> - This subroutine will execute a Fortran "CLOSE" on logical unit LUNIT,
!> even though subroutine openbf() didn't previously handle the corresponding
!> Fortran "OPEN" of the same file.
!> - It's a good idea to call this subroutine for every LUNIT that was
!> opened to the software via openbf(); however, it's especially
!> important to do so when writing/encoding a BUFR file, in order to
!> ensure that all output is properly flushed to LUNIT.
!>
