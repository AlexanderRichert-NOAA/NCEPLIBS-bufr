!> @file
!> @brief Add a tank receipt time to a BUFR message.

!> This subroutine reads an input message and outputs an equivalent
!> BUFR message with a tank receipt time added to Section 1.
!>
!> <p>The tank receipt time to be added must have been specified via
!> a previous call to subroutine strcpt().  This subroutine performs
!> the same function as subroutine strcpt() when the latter is called
!> with CF = 'Y', except that the latter subroutine operates on BUFR
!> messages internally within the software, whereas this subroutine
!> operates on a single BUFR message passed in via a memory array.
!>
!> @author J. Ator
!> @date 2009-03-23
!>
...
!> @param[out] MSGOT  -- integer(*): Copy of MSGIN with a tank
!>                       receipt time added to Section 1
!>
!> @remarks
!> - MSGIN and MSGOT must be separate arrays.
!> - MSGOT will be longer in length than MSGIN, so the user must allow
...
!	message will be 6 bytes longer than the input message.
...
!	Copy Sections 2, 3, 4 and 5 from the input array to the output array.
	...
	RETURN
900	CALL BORT('BUFRLIB: ATRCPT - OVERFLOW OF OUTPUT MESSAGE ARRAY; TRY A LARGER DIMENSION FOR THIS ARRAY')
